...
'''
# Generate the data.
X_train, X_test, Y_train, Y_test = fn.gen_train_test_data(cfg.datapath, cfg.train_test_split_ratio)
# Preprocess the data.
X_train_normalized = fn.do_minMaxNormalization(X_train)
X_test_normalized = fn.do_minMaxNormalization(X_test)
...
feature_importance_list = fn.cal_pertubation_importance(clf, X_test_normalized, Y_test, cfg.clf_name)
...
fn.save_dict(filename + '.csv', feature_dict)
fn.save_bar_chart(filename +'.png', cfg.feature_label_list, feature_importance_list, 'importance')
